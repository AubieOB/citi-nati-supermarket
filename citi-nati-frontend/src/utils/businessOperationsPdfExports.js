...
const formatLedgerDateTime = (value, isDateOnly = false) => {
  if (!value) return '-';
  ...
  if (isDateOnly) {
    ...
    if (!Number.isNaN(d.getTime())) {
      // Don't use toLocaleString to avoid timezone issues
      return String(value);
    }
  ...
  // Don't use toLocaleString to avoid timezone conversion issues
...
const drawMainDataTable = (doc, config, startY, headerContext) => {
  ...
  autoTable(doc, {
    startY,
    margin: { left, right, top: 16, bottom: 18 },
    head: [config.headers],
    showHead: config.showHead || 'everyPage',
...
const drawFooter = (doc, pageNumber, totalPages, generatedText) => {
  const pageHeight = doc.internal.pageSize.getHeight();
  const { left, right } = getContentBounds(doc);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(...COLOR_MUTED);
  doc.setFontSize(8);
  doc.text('Citi-Nati Supermarket Business Operations Report', left, pageHeight - 8);
  doc.text('Generated from Business Operations', left, pageHeight - 4);
  if (generatedText) {
    doc.text(`Exported: ${generatedText}`, right, pageHeight - 8, { align: 'right' });
  }
  doc.text(`Page ${pageNumber} of ${totalPages}`, right, pageHeight - 4, { align: 'right' });
};
...
const exportWithLayout = ({ reportTitle, viewLabel, periodText, summaryCards, metadataRows, dataTable, fileName }) => {
  ...
  for (let page = 1; page <= totalPages; page += 1) {
    doc.setPage(page);
    drawFooter(doc, page, totalPages, generatedText);
  }
...
export function exportStockMovementLedgerPdf({
  scopeLabel,
  filters = {},
  ledger = [],
  summary = {},
  ...
  const movementTypeLabel = filters.movementType ? String(filters.movementType).replace(/_/g, ' ') : 'All';
  const productFilterLabel = [filters.productCode, filters.productName].filter(Boolean).join(' / ') || 'All products';
  const periodLabel = titleCase(filters.periodType || 'day');
  const dateRangeLabel = (() => {
    const period = String(filters.periodType || '').toLowerCase();
    if (period === 'month') {
      return `${filters.month || 'Unknown'} ${filters.year || ''}`.trim();
    }
    if (period === 'year') {
      return filters.year ? String(filters.year) : 'Not set';
    }
    if (period === 'custom') {
      return `${toDate(filters.startDate)} - ${toDate(filters.endDate)}`;
    }
    return toDate(filters.date || filters.startDate || '');
  })();
  ...
  const metadataRows = [
    ...
    ['Period', periodLabel],
    ['Date / Range', dateRangeLabel || 'All'],
    ['Movement Type', movementTypeLabel],
    ['Product Filter', productFilterLabel],
    ['Generated At', formatGeneratedTimestamp()],
    ['Generated By', generatedBy || 'Admin'],
  ];

  const rows = ledger.map((row) => [
    formatLedgerDateTime(row.timestamp, row.isDateOnly),
    row.movementType || '-',
    row.referenceNo || '-',
    row.user || '-',
    row.productCode || '-',
    row.productName || '-',
    fmtCount(row.openingBalance ?? 0),
    fmtCount(row.qtyIn ?? 0),
    fmtCount(row.qtyOut ?? 0),
    fmtCount(row.balanceAfterTransaction ?? 0),
    row.closingBalance !== null && row.closingBalance !== undefined ? fmtCount(row.closingBalance) : '-',
    row.unitPrice != null ? fmtCurrency(row.unitPrice) : '-',
    row.lineAmount != null ? fmtCurrency(row.lineAmount) : '-',
  ]);

  const dataTable = {
    headers: ['Time', 'Type', 'Reference', 'User', 'Product Code', 'Product Name', 'Open Bal', 'Qty In', 'Qty Out', 'Balance', 'Close Bal', 'Unit Price', 'Amount'],
    rows,
    showHead: 'everyPage',
    styles: { overflow: 'linebreak', cellPadding: 3, fontSize: 8, valign: 'middle' },
    headStyles: {
      fillColor: toRgb(BRAND_PURPLE),
      textColor: [255, 255, 255],
      fontStyle: 'bold',
    ...
    columnStyles: {
      ...
      2: { cellWidth: 20, halign: 'center', overflow: 'ellipsize' },
      3: { cellWidth: 22, halign: 'center', overflow: 'ellipsize' },
      4: { cellWidth: 26, halign: 'left', overflow: 'ellipsize' },
      5: { cellWidth: 52, halign: 'left', overflow: 'linebreak' },
      6: { cellWidth: 14, halign: 'right', overflow: 'ellipsize' },
      7: { cellWidth: 10, halign: 'right', overflow: 'ellipsize' },
      8: { cellWidth: 10, halign: 'right', overflow: 'ellipsize' },
      9: { cellWidth: 14, halign: 'right', overflow: 'ellipsize' },
      10: { cellWidth: 14, halign: 'right', overflow: 'ellipsize' },
      11: { cellWidth: 20, halign: 'right', overflow: 'ellipsize' },
      12: { cellWidth: 24, halign: 'right', overflow: 'ellipsize' },
    },
    didParseCell: (data) => {
      if (data.section === 'body' && data.column.index === 1) {
        const movementText = String(data.cell.raw || '').toUpperCase();
        if (movementText === 'SALE') {
          data.cell.styles.textColor = [220, 53, 69];
        } else if (movementText.includes('INTAKE') || movementText === 'IN') {
  ...
  const dateLabel = localDateKey(new Date());
  exportWithLayout({
    reportTitle: 'Stock Movement Ledger',
    viewLabel: 'Stock Movement Ledger',
    periodText: `${periodLabel} • ${dateRangeLabel}`,
    summaryCards,
    metadataRows,
    dataTable,
    fileName: `stock-movement-ledger_${dateLabel}.pdf`,
  });
}
