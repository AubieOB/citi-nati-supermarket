...
const navigationTabStyle = (active, isDark) => ({
  ...
  background: active
    ? isDark
      ...
      : 'linear-gradient(135deg, #1e3a5f 0%, #2f67a8 100%)'
    : isDark
      ? '#1e1e1e'
      : '#e2e8f0',
  ...
  boxShadow: active
    ? isDark
      ...
      : '0 10px 22px rgba(47, 103, 168, 0.28)'
    : 'none',
});
...
function resolveLocationScopeLabel(filters = {}, selectedBranchCode = '', selectedLocationCode = '') {
  const locationCode = normalizeCode(filters.locationCode || selectedLocationCode);
  const branchCode = normalizeCode(filters.branchCode || selectedBranchCode);
...
const SalesReportsTab = ({
  ...
  selectedLocationId = null, // legacy compatibility only
  selectedBranchCode = '',
  ...
  useEffect(() => {
    setFilters((prev) => {
      if (
        prev.branchCode === canonicalScope.branchCode &&
        prev.locationCode === canonicalScope.locationCode &&
        prev.locationId === ''
      ) {
        return prev;
      }
  ...
  useEffect(() => {
    ...
    setFilters((prev) => {
      ...
      const branchCode = normalizeCode(drilldownRequest.branchCode || canonicalScope.branchCode);
      const locationCode = normalizeCode(drilldownRequest.locationCode || canonicalScope.locationCode);
  ...
  }, [canViewSalesBy, canViewSummary, canonicalScope, drilldownRequest]);
  ...
  const updateFilter = useCallback((key, value) => {
    setFilters((prev) => {
      ...
      if (key === 'branchCode' || key === 'locationCode') {
        next[key] = normalizeCode(value);
        next.locationId = '';
      }
  ...
  const runAutoRefresh = useCallback(async () => {
    ...
    try {
      await Promise.all([
        ...
        fetchProfitSummary({ background: true }),
      ]);
  ...
  const fetchAllRowsForView = useCallback(
    async (viewId) => {
      ...
      const pageSize = Number(
        stateForView.pagination?.pageSize ||
          stateForView.pagination?.take ||
          viewState?.[viewId]?.pageSize ||
          20
      );
  ...
  const handleExport = useCallback(
    async (format) => {
      if (!canExportReports) {
        await boAlert({
          title: 'Access denied',
          message: 'You do not have permission to export sales reports.',
          type: 'warning',
        });
        return;
      }
      ...
      try {
        if (format === 'pdf') {
          const activeViewLabelForExport =
        ...
        const message =
          error?.response?.data?.error || error?.message || `Failed to export ${format.toUpperCase()} report.`;
  ...
  const handleExportFullWorkbook = useCallback(async () => {
    if (!canExportFullWorkbook) {
      await boAlert({
        title: 'Access denied',
        message: 'You do not have permission to export full workbooks.',
        type: 'warning',
      });
      return;
    }
    ...
    try {
      await downloadFullBusinessWorkbook({
        filters: buildReportParams({
          branchCode: filters.branchCode || canonicalScope.branchCode,
          locationCode: filters.locationCode || canonicalScope.locationCode,
    ...
    canonicalScope.branchCode,
    canonicalScope.locationCode,
    filters.branchCode,
    filters.endDate,
    filters.locationCode,
    filters.startDate,
    filters.syncSourceCode,
  ]);
  ...
  const handleImportWorkbookFileChange = useCallback(
    async (event) => {
      ...
      try {
        const response = await importFullBusinessWorkbook({
          ...
          upsert: true,
          clearExisting: false,
          branchCode: filters.branchCode || canonicalScope.branchCode || null,
          locationCode: filters.locationCode || canonicalScope.locationCode || null,
          // Keep legacy value only for old backend compatibility.
          // Backend should prefer branchCode + locationCode where supported.
    ...
    return (
      <div style={{ display: 'grid', gap: '1rem' }}>
        <SalesSummaryCards
          ...
          profitLoading={profitSummaryLoading}
        />
        ...
        <div style={{ ...baseCardStyle, padding: '1.25rem' }}>
          ...
          <div style={{ marginTop: '0.85rem', display: 'flex', gap: '0.6rem', flexWrap: 'wrap' }}>
            {summaryMetaLine.length ? (
              summaryMetaLine.map((item) => (
                <span
                  key={item}
                  style={{
                ...
                <tr key={`${row.id}-${row.sourceInvoiceNo}`}>
                  <td style={tdStyle}>
                    ...
                    <div style={{ color: '#64748b', fontSize: '0.82rem' }}>
                      {row.syncSourceCode || row.branchCode || 'Unknown source'}
                    </div>
                  </td>
                  <td style={tdStyle}>
                    <div>{row.invoiceDate ? new Date(row.invoiceDate).toLocaleDateString() : 'N/A'}</div>
                    <div style={{ color: '#64748b', fontSize: '0.82rem' }}>
                      {formatInvoiceTimeDisplay(row.invoiceTime)}
                    </div>
                  </td>
            ...
            <tbody>
              {productsState.data.map((row) => (
                <tr key={`${row.productCode}-${row.productName}`}>
                  ...
                  <td style={tdStyle}>
                    {Number(row.totalQuantitySold || 0).toLocaleString('en-US', { maximumFractionDigits: 2 })}
                  </td>
                  ...
                  <td style={tdStyle}>
                    {row.estimatedMarginPct !== null && row.estimatedMarginPct !== undefined
                      ? `${Number(row.estimatedMarginPct).toFixed(2)}%`
                      : 'N/A'}
                  </td>
                </tr>
              ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
          {paymentsState.data.map((row) => (
            <div key={row.payMethod} style={{ ...baseCardStyle, padding: '1rem 1.1rem' }}>
              ...
              <div style={{ marginTop: '0.35rem', fontSize: '1.15rem', fontWeight: 800, color: '#0f172a' }}>
                {row.payMethod}
              </div>
              <div style={{ marginTop: '0.85rem', color: '#334155', fontWeight: 700, whiteSpace: 'nowrap' }}>
                {money(row.totalAmount)}
              </div>
              <div style={{ marginTop: '0.35rem', color: '#64748b', fontSize: '0.88rem' }}>
                {integer(row.invoiceCount)} invoice occurrences
              </div>
            </div>
          ))}
        </div>
        ...
        {paymentsState.totals && (
          <div style={{ ...baseCardStyle, padding: '1rem 1.1rem', display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'space-between' }}>
            <div>
              ...
              <div style={{ marginTop: '0.35rem', fontSize: '1.08rem', lineHeight: 1.1, fontWeight: 800, color: '#0f172a', whiteSpace: 'nowrap' }}>
                {money(paymentsState.totals.totalAmount)}
              </div>
            </div>
            <div>
              <div style={{ color: '#64748b', fontSize: '0.82rem', fontWeight: 700, textTransform: 'uppercase' }}>Invoice Count</div>
              <div style={{ marginTop: '0.35rem', fontSize: '1.08rem', lineHeight: 1.1, fontWeight: 800, color: '#0f172a', whiteSpace: 'nowrap' }}>
                {integer(paymentsState.totals.invoiceCount)}
              </div>
            </div>
          </div>
        )}
  ...
  const exportDisabled = summaryLoading || exportingExcel || exportingPdf || exportingFullWorkbook || importingFullWorkbook;

  const renderReportActionButtons = () => (
    <>
      {canExportReports && (
        <button
          type="button"
      ...
      {canExportFullWorkbook && (
        <button
          ...
          disabled={exportDisabled}
          style={{
            border: isAdminDarkTheme ? '1px solid #2f7f58' : '1px solid #86efac',
            background: isAdminDarkTheme ? '#153828' : '#f0fdf4',
            color: isAdminDarkTheme ? '#91e0b4' : '#166534',
            borderRadius: '10px',
            padding: '0.58rem 0.86rem',
            fontWeight: 800,
            cursor: exportDisabled ? 'not-allowed' : 'pointer',
      ...
      {(canImportFullWorkbook || canImportReports) && (
        <button
          type="button"
          onClick={handleChooseImportWorkbook}
          disabled={exportDisabled}
          style={{
            border: isAdminDarkTheme ? '1px solid #5b4b8a' : '1px solid #bfdbfe',
            background: isAdminDarkTheme ? '#2a2438' : '#eff6ff',
            color: isAdminDarkTheme ? '#d7cff5' : '#1e3a8a',
            borderRadius: '10px',
            padding: '0.58rem 0.86rem',
            fontWeight: 800,
            cursor: exportDisabled ? 'not-allowed' : 'pointer',
      ...
      {activeView !== 'summary' && isReportModalOpen && canViewSalesBy && (
        <div
          style={{
            ...
            display: 'grid',
            placeItems: 'center',
            padding: isReportModalMaximized ? '0.35rem' : '1rem',
          }}
        >
          <div
            style={{
              ...baseCardStyle,
              width: isReportModalMaximized ? 'calc(100vw - 0.7rem)' : 'min(1240px, 97vw)',
              height: isReportModalMaximized ? 'calc(100vh - 0.7rem)' : '90vh',
              maxHeight: 'none',
              overflow: 'auto',
              borderRadius: isReportModalMaximized ? '10px' : '18px',
              padding: '0.9rem',
            }}
          >
            <div
