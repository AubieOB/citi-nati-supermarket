...
const SupplierTransactionFormModal = ({
  ...
  const effectiveBranchCode = normalizeCode(selectedBranchCode || transaction?.branchCode);
  const effectiveLocationCode = normalizeCode(selectedLocationCode || transaction?.locationCode);
  ...
  const title = useMemo(
    () => (transaction ? 'Edit Supplier Transaction' : 'Add Supplier Transaction'),
    [transaction]
  );
  ...
  }, [isOpen, selectedLocationId, supplier, transaction]);
