    ...
    async (q) => {
      ...
      try {
        const resp = await api.get('/business-operations/goods-intake/lookup-products', {
          params: { q: query, ...scopeParams, take: 12 },
        });
    ...
    <div style={{ display: 'grid', gap: '1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button onClick={() => saveDraft('draft')} disabled={saving} style={{ padding: '0.6rem 0.9rem', borderRadius: 8, backgroundColor: '#2563eb', color: '#fff', border: 'none' }}>{saving ? 'Saving…' : 'Save Draft'}</button>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
        ...
        />
        <button onClick={() => searchProducts(searchTerm)} style={{ padding: '0.6rem 0.9rem', borderRadius: 8, border: '1px solid #cbd5e1', backgroundColor: '#fff' }}>{searchLoading ? 'Searching…' : 'Search'}</button>
        <button onClick={addManualRow} style={{ padding: '0.6rem 0.9rem', borderRadius: 8, border: '1px solid #cbd5e1', backgroundColor: '#fff' }}>Add Manual Row</button>
      </div>
