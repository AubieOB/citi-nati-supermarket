...
async function executeCreatePendingStockIntake(pool, payload, commandId) {
  ...
  var normalizedSupplierCode = Number(supplierCode);
  if (!isFinite(normalizedSupplierCode) || normalizedSupplierCode <= 0 || Math.floor(normalizedSupplierCode) !== normalizedSupplierCode) {
...
async function executeDeleteSupplier(pool, payload, commandId) {
  ...
  var rowsAffected = result.rowsAffected && result.rowsAffected[0];

  console.log('[SUPPLIER DELETE] DELETE_SUPPLIER executed', {
    commandId: commandId,
    posSupplierCode: posSupplierCode,
