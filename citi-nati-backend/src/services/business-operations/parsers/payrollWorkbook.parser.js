...
function parseLoanSheet(workbook, sheetName, warnings) {
  ...
  for (const row of dataRows) {
    ...
    loans.push({
      ...
      interestRate: parseNumber(findCellByAliases(row, headerMap, ['Loan Interest Rate', 'Interest Rate'])) ,
      accruedInterest: parseNumber(findCellByAliases(row, headerMap, ['Accrued Interest', 'Of which accrued interest'])),
      loanGrantedMonth: parseNumber(findCellByAliases(row, headerMap, ['Month loan granted'])),
      ...
      reason: cleanString(findCellByAliases(row, headerMap, ['Reason for Loan', 'Loan Reason'])),
      notes: cleanString(findCellByAliases(row, headerMap, ['Notes', 'Comment'])),
    });
  ...
  function parseTerminationFormBlocks() {
    ...
    for (let i = 0; i < rows.length; i += 1) {
      ...
      terminations.push({
        ...
        terminationType: 'resignation_or_dismissal',
        ...
        grossSettlementAmount: parseAmountSafe(findLabelValueInWindow(rows, i, i + 28, ['Total Gross Amount Due at End of Contract'])),
        outstandingLoanObligations: parseAmountSafe(findLabelValueInWindow(rows, i, i + 28, ['Less Outstanding Loan Obligations'])),
        netSettlementAmount: parseAmountSafe(findLabelValueInWindow(rows, i, i + 28, ['Total Net Amount Due at End of Contract'])),
  ...
  if (headerIndex >= 0) {
    ...
    for (const row of dataRows) {
      ...
      terminations.push({
        ...
        employeeName,
        terminationDate,
        terminationType: cleanString(findCellByAliases(row, headerMap, ['Termination Type', 'Type'])),
        ...
        halfPayDueInTerminationMonth: parseAmountSafe(findCellByAliases(row, headerMap, ['Half Pay Due in Month of Termination'])),
        amountPaidInTerminationMonth: parseAmountSafe(findCellByAliases(row, headerMap, ['Amount Paid in Month of Termination'])),
        leavePayAccruedDays: parseAmountSafe(findCellByAliases(row, headerMap, ['Leave Pay on Accrued Leave Days', 'Leave Accrued Days'])),
        leavePayAmount: parseAmountSafe(findCellByAliases(row, headerMap, ['Leave Pay Amount', 'Leave Pay on Accrued Leave Days'])),
        outstandingLoanObligations: parseAmountSafe(findCellByAliases(row, headerMap, ['Less Outstanding Loan Obligations', 'Outstanding Loan Obligations'])),
        grossSettlementAmount: parseAmountSafe(findCellByAliases(row, headerMap, ['Total Gross Amount Due at End of Contract', 'Gross Settlement Amount'])),
        netSettlementAmount: parseAmountSafe(findCellByAliases(row, headerMap, ['Total Net Amount Due at End of Contract', 'Net Settlement Amount'])),
  ...
  if (headerIndex >= 0) {
    ...
    for (const row of dataRows) {
      ...
      reengagements.push({
        employeeNo,
        employeeName,
        wageAtRetrenchment: parseAmountSafe(findCellByAliases(row, headerMap, ['Wages on retrenchment', 'Wage at Retrenchment'])),
  ...
  if (!reengagements.length) {
    ...
    for (let i = 0; i < rows.length; i += 1) {
      ...
      reengagements.push({
        ...
        employeeName,
        wageAtRetrenchment: parseAmountSafe(findLabelValueInWindow(rows, i, i + 18, ['Wages on retrenchment', 'Wage at Retrenchment'])),
...
function parsePayrollLikeSheets(workbook, sheetNames, warnings) {
  ...
  function pushPayrollEntry(sheetName, payrollMode, periodDescription, entry) {
    ...
    const normalizedEntry = {
      ...
      overtimeNormalHours: parseAmountSafe(entry.overtimeNormalHours),
      overtimeDoubleHours: parseAmountSafe(entry.overtimeDoubleHours),
      overtimeAmount: parseAmountSafe(entry.overtimeAmount),
      overtimeNormalAmount: parseAmountSafe(entry.overtimeNormalAmount),
      overtimeDoubleAmount: parseAmountSafe(entry.overtimeDoubleAmount),
      loanDeductionAmount: parseAmountSafe(entry.loanDeductionAmount),
      absenceDeductionAmount: parseAmountSafe(entry.absenceDeductionAmount),
      ...
      loanBalanceAtPayroll: parseAmountSafe(entry.loanBalanceAtPayroll),
      accruedInterestAtPayroll: parseAmountSafe(entry.accruedInterestAtPayroll),
      netPayMidPortion: parseAmountSafe(entry.netPayMidPortion),
      netPayEndPortion: parseAmountSafe(entry.netPayEndPortion),
  ...
  sheetNames.forEach((sheetName) => {
    ...
    dataRows.forEach((row) => {
      ...
      pushPayrollEntry(sheetName, payrollMode, periodDescription, {
        ...
        overtimeNormalHours: findCellByAliases(row, headerMap, ['Normal Overtime', 'Normal Overtime Worked']),
        overtimeDoubleHours: findCellByAliases(row, headerMap, ['Double Overtime', 'Double Overtime Worked']),
        overtimeAmount: findCellByAliases(row, headerMap, ['Overtime Amount', 'Overtime Claim']),
        overtimeNormalAmount: findCellByAliases(row, headerMap, ['Normal Overtime Amount']),
        overtimeDoubleAmount: findCellByAliases(row, headerMap, ['Double Overtime Amount']),
        loanDeductionAmount: findCellByAliases(row, headerMap, ['Loan Deduction', 'Loan Instalment', 'Loan Deduction Amount']),
        absenceDeductionAmount: findCellByAliases(row, headerMap, ['Absence Deduction', 'Days Absent']),
        otherDeductionAmount: findCellByAliases(row, headerMap, ['Other Deduction', 'Other Deductions']),
        ...
        loanBalanceAtPayroll: findCellByAliases(row, headerMap, ['Loan Balance', 'Loan Outstanding']),
        accruedInterestAtPayroll: findCellByAliases(row, headerMap, ['Accrued Interest', 'Accrued Interest To-Date']),
        netPayMidPortion: findCellByAliases(row, headerMap, ['Net Pay mid and end of Month', 'Net Pay Mid Portion']),
        netPayEndPortion: findCellByAliases(row, headerMap, ['Net Pay End Portion']),
        notes: cleanString(findCellByAliases(row, headerMap, ['Notes', 'Comment'])),
      });
    });
    ...
    for (let i = 0; i < rows.length; i += 1) {
      ...
      pushPayrollEntry(sheetName, payrollMode, periodDescription, {
        ...
        payeAmount: paye,
        loanBalanceAtPayroll: findLabelValueInWindow(rows, i, i + 18, ['Loan Balance', 'Loan Outstanding']),
        accruedInterestAtPayroll: findLabelValueInWindow(rows, i, i + 18, ['Accrued Interest', 'Accrued Interest To-Date']),
        netPayMidPortion: findLabelValueInWindow(rows, i, i + 18, ['Net Pay mid and end of Month']),
