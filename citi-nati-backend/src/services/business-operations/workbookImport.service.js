  ...
  if (includeSection(sectionSet, 'payrollEntries') && parsed.payrollEntries.length) {
    ...
    parsed.payrollEntries.forEach((entry) => {
      ...
      normalizedEntries.push({
        ...
        overtimeNormalHours: entry.overtimeNormalHours,
        overtimeDoubleHours: entry.overtimeDoubleHours,
        overtimeAmount: entry.overtimeAmount,
        overtimeNormalAmount: entry.overtimeNormalAmount,
        overtimeDoubleAmount: entry.overtimeDoubleAmount,
        loanDeductionAmount: entry.loanDeductionAmount,
        absenceDeductionAmount: entry.absenceDeductionAmount,
        ...
        loanBalanceAtPayroll: entry.loanBalanceAtPayroll,
        accruedInterestAtPayroll: entry.accruedInterestAtPayroll,
        netPayMidPortion: entry.netPayMidPortion,
        netPayEndPortion: entry.netPayEndPortion,
        notes: entry.notes,
      });
    });
    ...
    const normalized = parsed.terminations
      .map((row) => {
        ...
        return {
          ...
          halfPayDueInTerminationMonth: row.halfPayDueInTerminationMonth,
          amountPaidInTerminationMonth: row.amountPaidInTerminationMonth,
          leavePayAccruedDays: row.leavePayAccruedDays,
          leavePayAmount: row.leavePayAmount,
          outstandingLoanObligations: row.outstandingLoanObligations,
          grossSettlementAmount: row.grossSettlementAmount,
  ...
  if (includeSection(sectionSet, 'reengagements') && parsed.reengagements.length) {
    ...
    const normalized = parsed.reengagements
      .map((row) => {
        ...
        return {
          employeeId,
          linkedTerminationId: row.linkedTerminationId,
          wageAtRetrenchment: row.wageAtRetrenchment,
